...
public class ExceptionControllerAdvice {
    ...
    public ResponseEntity<ErrorInfo> CourierExceptionHandler(CourierException exception){
        ErrorInfo error = new ErrorInfo();
    ...
    public ResponseEntity<ErrorInfo> ExceptionHandler2(Exception ex){
        ...
        if(ex instanceof MethodArgumentNotValidException){
            ...
            errorMsg = manve.getBindingResult().getAllErrors().stream().map(DefaultMessageSourceResolvable::getDefaultMessage).collect(Collectors.joining(","));
        } else {
